...
def encode_features():
    ...
    try:
        ...
        df_en.to_sql(name='features_inference', con=connection, if_exists='replace', index=False)
        return "encode_features successful"
    except Error as e:
        raise RuntimeError("Failure in encode_features: " + str(e))
    finally:
        connection.close()
...
def get_models_prediction():
    ...
    try:
        ...
        # read the new data
        connection = sqlite3.connect(DB_PATH+DB_FILE_NAME)
        df_fea_inf = pd.read_sql('select * from features_inference', connection)
        ...
        df_fea_inf.to_sql(name='predicted_values', con=connection, if_exists='replace', index=False)
        return "get_models_prediction successful"
    except Error as e:
        raise RuntimeError("Failure in get_models_prediction: " + str(e))
    finally:
        connection.close()
...
def prediction_ratio_check():
    ...
    try:
        # read the input data
        connection = sqlite3.connect(DB_PATH+DB_FILE_NAME)
        df_pred_val = pd.read_sql('select * from predicted_values', connection)
        ...
        print(FILE_PATH+'prediction_distribution.txt')
        ...
        with open(FILE_PATH+'prediction_distribution.txt', 'a') as f:
            f.write(st+"\n")
            print(st)
        return "prediction_ratio_check successful"
    except Error as e:
        raise RuntimeError("Failure in prediction_ratio_check: " + str(e))
    finally:
        connection.close()
###############################################################################
...
def input_features_check():
    ...
    try:
        # read the input data
        connection = sqlite3.connect(DB_PATH+DB_FILE_NAME)
        df_inf = pd.read_sql('select * from features_inference', connection)
        ...
        return "input_col_check successful"
    except Error as e:
        raise RuntimeError("Failure in input_col_check: " + str(e))
    finally:
        connection.close()
